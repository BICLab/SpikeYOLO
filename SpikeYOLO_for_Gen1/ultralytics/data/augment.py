...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if len(img.shape) == 4:  # 处理动态数据集:   难道是直接reshape的？
            ...
            T, H, W, C = img.shape
            img_final = np.zeros([T, new_unpad[1]+top+bottom, new_unpad[0]+left+right, C])  # 进行填充 这里进行修改，根据new_unpad而不是正方形
            for i in range(T):
                img_final[i] = cv2.copyMakeBorder(img[i], top, bottom, left, right, cv2.BORDER_CONSTANT,  #这里的shape还是THWC
                                                  value=(114, 114, 114))  # add border
            img = img_final

        else:
            img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,  #将函数扩展为可以被32整除的格式(根据参数可能拓展为矩形) 这里应该也是hwc
                                     value=(114, 114, 114))  # add border
