...
# 241版本，还是用heaviside，但是可以理解为阈值为0
...
# #             BB = heaviside(x)
# #             CC = torch.clamp(torch.floor(x), min=0, max=d)
# #             if BB.equal(CC):
# #                 print("True")
# #             else:
# #                 print("False")
...
#         return heaviside(x)
# #         print("torch.clamp(torch.floor(x), min=0, max=d:",torch.clamp(torch.floor(x), min=0, max=d))
# #         return torch.clamp(torch.floor(x), min=0, max=d)  # 这里将两个4改为1，来对比t1时候的snn
...
class quant1(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return torch.round(torch.clamp(input, min=0, max=1))  # 这里将两个4改为1，来对比t1时候的snn
...
class quant2(torch.autograd.Function):
    ...
    def forward(ctx, input):
        #         print("quant")
        ctx.save_for_backward(input)
        return torch.round(torch.clamp(input, min=0, max=2))  # 这里将两个4改为1，来对比t1时候的snn
...
class quant3(torch.autograd.Function):

    @staticmethod
    def forward(ctx, input):
        #         print("quant")
        ctx.save_for_backward(input)
        return torch.round(torch.clamp(input, min=0, max=3))  # 这里将两个4改为1，来对比t1时候的snn
...
class quantsigmoid(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, alpha, d):
        # d = 2 #todo: 将d作为超参数移到函数外部
        if x.requires_grad:
            ctx.save_for_backward(x)
            ctx.d = d
            ctx.alpha = alpha
        return torch.round(torch.clamp(x, min=0, max=d))  # 这里将两个4改为1，来对比t1时候的snn

    @staticmethod
    def backward(ctx, grad_output):
        grad_x = None
        # 下面[0]代表tuple第一个元素，而不是tensor第一个维度；另外可以通过写bug的方法在这里停顿
        # 合理的方法是得到一组sgax，在范围[0.5,d-0.5]中取值0.5，在范围以外通过sgax取值
        if ctx.needs_input_grad[0]:
            symmetry = ctx.d/2   #得到全部数据的对称轴
            # print("ctx.saved_tensors:", ctx.saved_tensors.shape)
            absolute = torch.where(ctx.saved_tensors[0] > symmetry,symmetry*2-ctx.saved_tensors[0],ctx.saved_tensors[0]).to(torch.float32) #首先做对称处理，将对称轴以上的元素对称过来
#             print("absolute:",absolute.type())
            absolute = (4*absolute-2).sigmoid_()
#             print("BB:",BB.type())
            sgax = torch.where(absolute>=0.5,absolute * 0 + 0.5,absolute)  #todo: 大于0.5的直接取0.5(对应画图的平坦区域)，小于0.5的用sigmoid函数计算  不知道为什么这里直接写0.5在服务器上就会报数据不匹配错误
#             sgax = (ctx.saved_tensors[0] * 4).sigmoid_()
            grad_x = grad_output * (1. - sgax) * sgax * 4

        return grad_x, None, None # 两个None代表alpha和d的梯度为0.如果多加一个超参，这里也要多加个None
