...
from MOD_utils.model import create_model, load_model, save_model, load_coco_pretrained_model, load_imagenet_pretrained_model
from trainer.logger import Logger
from datasets.init_dataset import get_dataset
from trainer.MOD_Trainer import MODTrainer
...
def main(opt):
    ...
    trainer = MODTrainer(opt, model, optimizer)
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.auto_stop:
            ...
            if opt.model != '':
                opt.model = os.path.join(opt.model, model_name)
            if opt.flow_model != '':
